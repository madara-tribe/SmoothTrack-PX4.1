...
static inline void applyGlobalPreproc(const cv::Mat &src_bgr, double gamma, bool use_clahe,
                                      double sharp_amount, cv::Mat &dst_bgr) {
  CV_Assert(!src_bgr.empty() && src_bgr.type() == CV_8UC3);
  dst_bgr = src_bgr.clone();
  // gamma
  if (std::abs(gamma - 1.0) > 1e-3) {
    cv::Mat f;
    dst_bgr.convertTo(f, CV_32F, 1.0 / 255.0);
    cv::pow(f, gamma, f);
  ...
  if (use_clahe) {
    cv::Mat lab;
    cv::cvtColor(dst_bgr, lab, cv::COLOR_BGR2Lab);
  ...
  if (sharp_amount > 1e-6) {
    cv::Mat blur;
    cv::GaussianBlur(dst_bgr, blur, cv::Size(0, 0), 1.0);
