...
// inline to avoid ODR issues
inline std::pair<double, double> computeFov(double diag_fov_deg, int width, int height) {
...
private:
  ...
  double max_step_deg_ = 8.0;
  bool   center_on_start_ = false;
  std::string tracker_type_ = "KCF";
  bool   enforce_bgr8_ = true;
