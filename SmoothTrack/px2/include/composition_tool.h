...
static inline ThirdsTarget parseThirdsTarget(std::string s) {
  ...
  if (s == "left")
    return ThirdsTarget::LEFT;
  if (s == "right")
    return ThirdsTarget::RIGHT;
  if (s == "auto")
    return ThirdsTarget::AUTO;
  return ThirdsTarget::CENTER;
}

static inline double thirdsX(int img_w, ThirdsTarget t, double bbox_cx /* >=0 */,
                             bool use_auto_nearest = true) {
  const double left = img_w / 3.0;
  const double center = img_w / 2.0;
  const double right = (2.0 * img_w) / 3.0;
  switch (t) {
  case ThirdsTarget::LEFT:
    return left;
  case ThirdsTarget::CENTER:
    return center;
  case ThirdsTarget::RIGHT:
    return right;
  case ThirdsTarget::AUTO:
  default:
    if (!use_auto_nearest || bbox_cx < 0.0)
      return center;
    {
      double xs[3] = {left, center, right};
      double best = center, bd = std::numeric_limits<double>::infinity();
      for (double xi : xs) {
        double d = std::abs(bbox_cx - xi);
        if (d < bd) {
          bd = d;
          best = xi;
        }
...
static inline void drawThirdsOverlay(cv::Mat &img, const cv::Scalar &grid_color = {0, 255, 255},
                                     const cv::Scalar &mark_color = {0, 0, 255}, int target_x = -1,
                                     int mark_y = -1) {
